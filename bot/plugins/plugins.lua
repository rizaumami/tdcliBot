...
--------------------------------------------------------------------------------
  ...
  local function run(msg, matches)
    ...
    local receiver = tostring(msg.chat_id_)
    ...
    if isSudo(msg.sender_user_id_) then
      if matches[1] == 'setkey' then
        if pluginExists(plugin, plugins_dir) then
          ...
          sendText(msg.chat_id_, msg.id_, missing)
        end
      end
      ...
      if matches[3] == 'chat' then
        if matches[1] == 'enable' then
          ...
          if not _config.plugins.disabled_on_chat then
            return sendText(msg.chat_id_, msg.id_, _msg("There aren't any disabled plugins"))
          end

          if not _config.plugins.disabled_on_chat[receiver] then
            return sendText(msg.chat_id_, msg.id_, _msg("There aren't any disabled plugins for this chat"))
          end

          if not _config.plugins.disabled_on_chat[receiver][plugin] then
            return sendText(msg.chat_id_, msg.id_, _msg('This plugin is not disabled'))
          end
        ...
        if matches[1] == 'disable' then
          ...
          if not pluginExists(plugin, plugins_dir) then
            return sendText(msg.chat_id_, msg.id_, _msg("Plugin doesn't exists"))
          end
