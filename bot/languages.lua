-- Modified from https://github.com/RememberTheAir/GroupButler/blob/master/languages.lua

local directory = "locales"   -- locales (*.po) files folder
...
local function parse(filename)
  ...
  for line in io.lines(filename) do
    ...
    elseif state == 'msgstr' and input == 'fuzzy' then
      if msgstr ~= '' then result[msgid:gsub('^\n', '')] = msgstr end
      if not _config.language.allow_fuzzy_translations then
        state = 'ign_msgid'
      end
...
local pocale = util.scanDir(directory)
local langs = _config.language.available_languages

-- Scan locales folder on every boot
for po = 1, #pocale do
  if not langs[pocale[po]] then
    langs[po] = pocale[po]:gsub('.po', '')
  end
end
...
for b = 1, #langs do
  strings[langs[b]] = parse(string.format('%s/%s.po', directory, langs[b]))
end
